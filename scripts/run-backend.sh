...
STT_MODEL_DIR="${DICTOPHONE_MODEL_DIR:-$HOME/.local/share/dictophone/models/whisper-tiny-fp16-ov}"
POLISH_MODEL_DIR="${DICTOPHONE_POLISH_MODEL_DIR:-$HOME/.local/share/dictophone/models/qwen2.5-1.5b-instruct-int4-ov}"
...
if [[ -d "$STT_MODEL_DIR" ]]; then
  export DICTOPHONE_MODEL_ID="${DICTOPHONE_MODEL_ID:-OpenVINO/whisper-tiny-fp16-ov}"
  export DICTOPHONE_MODEL_DIR="$STT_MODEL_DIR"
fi

if [[ -d "$POLISH_MODEL_DIR" ]]; then
  export DICTOPHONE_POLISH_MODEL_ID="${DICTOPHONE_POLISH_MODEL_ID:-OpenVINO/Qwen2.5-1.5B-Instruct-int4-ov}"
